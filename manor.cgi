...
$disp.="<br><BIG>●".l("荘園状況")."</BIG><br><br>";
$disp.=$TB.$TR;
$disp.=$TDB.l("種").$TDB.l("販売在庫").$TDB.l("販売価格").$TDB.l("自店保有").$TD.$TDB.l("収穫物").$TDB.l("買取価格").$TDB.l("自店保有").$TRE;
...
foreach my $i(0..$#MANOR)
	...
	$disp.=$TD.($MANORLORD->{"count$i"} + 0)." ".l("個");
	...
	$disp.=$TD.($DT->{_seed}->{"ripe$i"} + 0)." ".l("個").$TRE;
...
$disp.=<<"HTML";
	...
	$USERPASSFORM
<BIG>●${\l('収穫物売却')}</BIG>： ${\l('収穫物を領主の荘園に ')}
<INPUT TYPE=SUBMIT VALUE="${\l('買い取ってもらう')}">
	</FORM>
HTML
	}
...
if ($STATE->{leader}==$DT->{id})
	...
	<INPUT TYPE=SUBMIT VALUE='${\l('荘園を管理する')}'>
	</FORM>
STR
	}
