...
$TB$TR
$TD$image[0]$TD
<SPAN>${\l('ギルド受付')}</SPAN>：${\l('入団するには，団長または軍師の許可が必要です。')}<br>
${\l('入りたいギルドの入団条件を参照し，問い合わせてみるとよいでしょう。')}
$TRE$TBE<br>
HTML
$disp.=l("ギルド戦中は入団できません"),return if ($DTevent{guildbattle});
my $join="";
...
$MYFORM$USERPASSFORM
<BIG>●$GUILD_DETAIL{$guild}->{shortname}</BIG>：${\l('入団許可が出ています。 ')}
<INPUT TYPE=HIDDEN NAME=edit VALUE="join">
<INPUT TYPE=HIDDEN NAME=guild VALUE="$guild">
<INPUT TYPE=SUBMIT VALUE="${\l('入団する')}">
</form><br>
HTML
	}
$disp.=($join) ? $join : l("入団許可が出ていません");
}
...
$TB$TR
$TD$image[0]$TD
<SPAN>${\l('ギルド受付')}</SPAN>：${\l('誰の入団を許可しますか？')}<br>
${\l('一度許可すると，撤回はできないのでご注意ください。')}
$TRE$TBE<br>
HTML
$disp.=l("ギルド戦中は許可できません"),return if ($DTevent{guildbattle});
...
foreach(0..$#OtherDir)
	...
	my $pg=$OtherDir[$_];
	$disp.=( ($_ % $r) ? "<br>" : $TD);
	$disp.="$Tname{$pg} <SELECT NAME=$pg><OPTION VALUE=\"\">".l("選択");
...
<INPUT TYPE=SUBMIT VALUE="${\l('入団を許可する')}">
</FORM>
HTML
}
...
$TB$TR
$TD$image[0]$TD
<SPAN>${\l('ギルド受付')}</SPAN>：${\l('入団手続を完了しました。')}<br>
${\l('ギルドの作戦室などであいさつしておくとよいでしょう。')}
$TRE$TBE<br>
HTML
}
