...
$img_ex[0]=GetMenuTag('lord','<acronym title="óÃéÂì@">'.$i[2].'</acronym>');
$img_ex[1]=GetMenuTag('shop-a','<acronym title="è§ìXí ÇË"><IMG WIDTH="96" HEIGHT="80" SRC="'.$IMAGE_URL.'/map/mapshop.png"><IMG WIDTH="96" HEIGHT="80" SRC="'.$IMAGE_URL.'/map/mapshop.png"></acronym>');
...
foreach(@DT)
	...
	$logmemb .= $_->{shopname}."ÅC ";
	}
$logmemb = substr($logmemb,0,(length($logmemb)-3)) if ($logmemb);
$logmemb = "Ç»Çµ" if !$logmemb;

$disp.=<<"HTML";
<table width=256>$TR$TDB<small>åªç›äàìÆíÜÇÃìX</small>$TRE
