...
sub CheckBride
{
	...
	my $MAXADD=$buf[0];
	$place[100]=$space.GetMenuTag('bride','<acronym title="ã≥âÔ"><IMG WIDTH=112 HEIGHT=80 SRC="'.$IMAGE_URL.'/map/house1d.png"></acronym>');
	...
	foreach my $cnt(0..$#bride)
		...
		undef $BRIDE[$cnt],next if !defined($id2idx{$BRIDE[$cnt]->{ida}}) || !defined($id2idx{$BRIDE[$cnt]->{idb}});	# ï¬ìXÅCà⁄ì]Ç…ÇÊÇÈè¡ñ≈ÅB
		if ($BRIDE[$cnt]->{place})
			{
			my $n=$DT[$id2idx{$BRIDE[$cnt]->{ida}}]->{name}."&".$DT[$id2idx{$BRIDE[$cnt]->{idb}}]->{name}."ëÓ";
	...
	foreach(101..$MAXADD)
		{
		next if $place[$_];
		$place[$_]='<acronym title="èZëÓópín"><IMG WIDTH="128" HEIGHT="80" SRC="'.$IMAGE_URL.'/map/housez.png"></acronym>';
