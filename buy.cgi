...
$disp.=$TR.$TDB;
$disp.=(($ITEM->{flag}=~/h/)? l("名前") : l("商品"));
$disp.=$TD.GetTagImgItemType($itemno).$ITEM->{name}.$TRE;
$disp.=$TR.$TDB.$TD.$ITEM->{info}.$TRE;
$disp.=$TR.$TDB.l("価格").$TD.'@'.GetMoneyString($baseprice).$TRE;
$disp.=$TR.$TDB.(l("ギルド内割引価格","ギルド間割増価格"))[$guild-1].$TD.'@'.GetMoneyString($saleprice).$TRE if $guild>0;
$disp.=$TR.$TDB.l("ギルド資金不足".$TD."ギルド内割引補助はありません").$TRE if $guild==-1;
$disp.=$TR.$TDB.l("販売在庫数").$TD.$stock.$ITEM->{scale}.$TRE;
$disp.=$TR.$TDB.l('自店保有数').$TD.($DT->{item}[$itemno-1]+0).$ITEM->{scale}.$TRE;
$disp.=$TBE;

if ($ITEM->{flag}!~/s/) {		# s 陳列不可
	$disp.="<SPAN>※".l('この商品は陳列しても売れません')."</SPAN><br>" if ($ITEM->{popular}==0);
	$disp.="<SPAN>※".l('この商品は陳列してもほとんど売れません')."</SPAN><br>" if ($ITEM->{popular} > 800000);
	}
$disp.="<hr width=500 noshade size=1>";

if($DT->{item}[$itemno-1]>=$ITEM->{limit})
	{$disp.='<BR>'.l('もう倉庫は%1が一杯です',$ITEM->{name}).'<BR>';}
...
else
{
	...
	$msg{1000}=1000;
	$msg{10000}=10000;
	$msg{$stock}="$stock(".l('全部').")";
	$msg{$limit}="$limit(".l('倉庫最大').")";
  ...
  if ($ITEM->{flag}=~/h/) {  $disp.="<INPUT TYPE=SUBMIT VALUE='".l("雇う")."'>";  }
         else 	{ $disp.="<INPUT TYPE=SUBMIT VALUE='".l('買う')."'>"; }

	$disp.="<br>(".l('消費時間').":".GetTime2HMS($usetime).")";
	$disp.="</FORM>";
}
